...
#b
X_train = DATA_concatenate(Creat_dataset('P'), Creat_dataset('Y'))/255
X_train = X_train.reshape(-1, 32, 32, 1)
y_train = DATA_concatenate(Creat_label(Creat_dataset('P'), label=1), Creat_label(Creat_dataset('Y'), label=0))

X_test = DATA_concatenate(Creat_test('P'), Creat_test('Y'))/255
X_test = X_test.reshape(-1, 32, 32, 1)
y_test = DATA_concatenate(Creat_label(Creat_test('P'), label=1), Creat_label(Creat_test('Y'), label=0))
...
print('For training data (P vs Y), the loss is %s , the accuracy is %s' % (loss, acc))

print('Testing mse:', mean_squared_error(y_test, y_pred))
loss, acc = CNN.evaluate(X_test, y_test)
print('For testing data (P vs Y), the loss is %s , the accuracy is %s' % (loss, acc))